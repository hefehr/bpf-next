				...
				struct bpf_prog *xdp_prog)
{
	...
	if (!(dev->xdp_features & XDP_ACT_NDO_XMIT))
		return -EOPNOTSUPP;

	if (unlikely(!(dev->xdp_features & XDP_ACT_NDO_XMIT_SG) &&
...
static bool is_valid_dst(struct bpf_dtab_netdev *obj, struct xdp_frame *xdpf)
{
	...
	if (!(obj->dev->xdp_features & XDP_ACT_NDO_XMIT))
		return false;

	if (unlikely(!(obj->dev->xdp_features & XDP_ACT_NDO_XMIT_SG) &&
		     xdp_frame_has_frags(xdpf)))
		return false;
						    ...
						    unsigned int idx)
{
	...
	if (!(READ_ONCE(dev->dev->xdp_features) & XDP_ACT_NDO_XMIT)) {
		ret = -EOPNOTSUPP;
		goto err_put_dev;
	}
