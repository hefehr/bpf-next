...
static int get_xdp_feature(const char *arg)
{
	if (!strcmp(arg, "XDP_PASS"))
		return XDP_FEATURES_FIELD_FLAG(XDP_ACT_PASS_BIT);
	else if (!strcmp(arg, "XDP_DROP"))
		return XDP_FEATURES_FIELD_FLAG(XDP_ACT_DROP_BIT);
	else if (!strcmp(arg, "XDP_ABORTED"))
		return XDP_FEATURES_FIELD_FLAG(XDP_ACT_ABORTED_BIT);
	else if (!strcmp(arg, "XDP_REDIRECT"))
		return XDP_FEATURES_FIELD_FLAG(XDP_ACT_REDIRECT_BIT);
	else if (!strcmp(arg, "XDP_NDO_XMIT"))
		return XDP_FEATURES_FIELD_FLAG(XDP_ACT_NDO_XMIT_BIT);
	else if (!strcmp(arg, "XDP_TX"))
		return XDP_FEATURES_FIELD_FLAG(XDP_ACT_TX_BIT);
...
{
	switch (feature) {
	case XDP_FEATURES_FIELD_FLAG(XDP_ACT_PASS_BIT):
		return YELLOW("XDP_PASS");
	case XDP_FEATURES_FIELD_FLAG(XDP_ACT_DROP_BIT):
		return YELLOW("XDP_DROP");
	case XDP_FEATURES_FIELD_FLAG(XDP_ACT_ABORTED_BIT):
		return YELLOW("XDP_ABORTED");
	case XDP_FEATURES_FIELD_FLAG(XDP_ACT_TX_BIT):
		return YELLOW("XDP_TX");
	case XDP_FEATURES_FIELD_FLAG(XDP_ACT_REDIRECT_BIT):
		return YELLOW("XDP_REDIRECT");
	case XDP_FEATURES_FIELD_FLAG(XDP_ACT_NDO_XMIT_BIT):
...
{
	env.feature = XDP_FEATURES_FIELD_FLAG(XDP_ACT_PASS_BIT);
	env.ifindex = -ENODEV;
